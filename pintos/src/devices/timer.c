...
/*new*/
bool sleep_less_func(const struct list_elem* a_elem, const struct list_elem* b_elem, void* aux);
...
timer_sleep (int64_t ticks) 
{
  ...
  t->waking_ticks = start + ticks;
  
  old_level = intr_disable();
  list_insert_ordered(&sleeping_list, &t->elem, sleep_less_func, NULL);
  thread_block();
  intr_set_level (old_level);
